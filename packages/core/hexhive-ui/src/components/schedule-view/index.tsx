...
export const ScheduleView: React.FC<ScheduleViewProps> = (props) => {
  ...
  const changeWeek = (week: Date) => {
    let params = [
      ...
      moment(week).clone().endOf("isoWeek"),
    ];
    setDate(moment(week).startOf("isoWeek"));
    setParams(params);

    props.onHorizonChanged(
      new Date(params[0].valueOf()),
      new Date(params[1].valueOf())
    );
  };
  ...
  const renderHeader = () => {
    ...
    return (
      <Box overflow="hidden" direction="column" className="week-header">
        <Box
    ...
    for (var i = 0; i < 7; i++) {
      ...
      headers.push(
        <Box
          ...
          flex
          className={
            currentDay == renderTime(i, "DD") &&
            currentMonth == renderTime(i, "MM")
              ? " week-day-header week-day-header-current"
              : "week-day-header"
          }
        >
          <Box>{renderTime(i, "ddd")}</Box>
          <Box>{renderTime(i, "DD/MM")}</Box>
        </Box>
      );
    }
    return headers;
  ...
  return (
    ...
    >
      <ScheduleContainer header={renderHeader()}>
        {props.isLoading ? (
