...
export default (neo: Session, fileManager: FileManager, taskRegistry: TaskRegistry) => {
    ...
    router.route('/run')
        .post(async (req, res) => {
            let {pipeline, params} = req.body;
            await neo.writeTransaction(async (tx) => {
                let param_query = await tx.run(`
                ...
                let input_params = param_query.records.map((x) => x.get(0).properties)

                //Param = {type, key, urn?}

                let parameters = input_params.map((x) => ({key: x.key, urn: `ipfs://${params[x.key]}`}))
                ...
                let resources = await Promise.all(parameters.map(async (param) => {
